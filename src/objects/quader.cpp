...
bool Quader::colsQuader(double SphereX, double SphereZ){
	SphX = SphereX, SphZ = SphereZ;
	if(SphX-.2 < x && SphX+.2 > x){  		//x Wert  des Objektes überprüft mit Kugelradius .2
		if(SphZ-.2 < x && SphZ+.2 > x ){ 	//z Wert des Objekt überprüft
			collisionZ = true;
			}else{
				collisionZ = false;
			}								//
	...
	if(collisionX && collisionZ){
			std::cout << 'Kollision mit Cube Object';
			return true;
		} else {
			return false;
		}
